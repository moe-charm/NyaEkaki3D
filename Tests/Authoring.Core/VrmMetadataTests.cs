...
internal static partial class Program
{
    static void RunVrmMetadataTests()
    {
        ...
        Test("VRM SpringBone inventory reads bounded VRM 1.0 and 0.x chains", () =>
        {
            ...
            modernSpring["colliders"][0]["shape"]["sphere"]["offset"] = new JArray(1, 2, 3);
            ((JArray)modernSpring["colliders"]).Add(modernSpring["colliders"][0].DeepClone());
            ((JArray)modernSpring["colliders"]).Add(new JObject { ["node"] = 2, ["shape"] = new JObject { ["capsule"] = new JObject { ["radius"] = .1, ["tail"] = new JArray(4, 5, 6), ["offset"] = new JArray(-1, 0, .2) } } });
            modernSpring["colliderGroups"][0]["colliders"] = new JArray(0, 1, 2);
            modernSpring["springs"][0]["joints"][0]["gravityDir"] = new JArray(2, -3, 4);
            ...
            var session = VrmSpringSession.Create(modern); True(session.HasCompleteDetails); var sessionBytes = VrmSpringSessionCodec.Write(session); var reopenedSession = VrmSpringSessionCodec.Read(sessionBytes); Equal(modern.SourceHash, reopenedSession.SourceHash); True(modern.Authors.SequenceEqual(reopenedSession.Authors)); Equal(2, reopenedSession.Authors.Count); Equal(1, reopenedSession.SpringBones.Count); Equal(2, reopenedSession.SpringBones[0].Joints.Count); Equal(3, reopenedSession.ColliderGroups[0].ColliderNodeIndices.Count); Equal(0, reopenedSession.ColliderGroups[0].ColliderNodeIndices[0]); True(sessionBytes.SequenceEqual(VrmSpringSessionCodec.Write(reopenedSession))); True(reopenedSession.ColliderGroups[0].ColliderNodeIndices.SequenceEqual(new[] { 0, 0, 2 })); Near(1, reopenedSession.SpringBones[0].Joints[1].Stiffness); Near(.5f, reopenedSession.SpringBones[0].Joints[1].DragForce);
            SpringPointNear(new NyaForge.Authoring.Vec3(2, -3, 4), reopenedSession.SpringBones[0].Joints[0].GravityDirection.Value); SpringPointNear(new NyaForge.Authoring.Vec3(0, -1, 0), reopenedSession.SpringBones[0].Joints[1].GravityDirection.Value); SpringPointNear(new NyaForge.Authoring.Vec3(1, 2, 3), reopenedSession.ColliderGroups[0].Shapes[1].Offset.Value); Equal("capsule", reopenedSession.ColliderGroups[0].Shapes[2].Kind); SpringPointNear(new NyaForge.Authoring.Vec3(4, 5, 6), reopenedSession.ColliderGroups[0].Shapes[2].Tail.Value);
            ...
            legacySecondary["boneGroups"][0]["gravityDir"] = new JObject { ["x"] = 1, ["y"] = 0, ["z"] = -1 };
            ...
            var legacyBytes = VrmSpringSessionCodec.Write(VrmSpringSession.Create(legacy)); var legacySession = VrmSpringSessionCodec.Read(legacyBytes); True(legacySession.HasCompleteDetails); SpringPointNear(new NyaForge.Authoring.Vec3(1, 0, -1), legacySession.SpringBones[0].Joints[0].GravityDirection.Value); True(legacySession.ColliderGroups[0].ColliderNodeIndices.SequenceEqual(new[] { 0, 0 })); True(legacyBytes.SequenceEqual(VrmSpringSessionCodec.Write(legacySession)));
