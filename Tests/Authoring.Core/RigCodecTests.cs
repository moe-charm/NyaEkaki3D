...
internal static partial class Program
{
    static void RunRigCodecTests()
    {
        ...
        Test("serialized multi-influence binding preserves exact float32 identity", () =>
        {
            string root = GraphId(), child = GraphId(), leaf = GraphId(), tip = GraphId();
            var skeleton = new SkeletonDefinition(new[]
            {
                new BoneDefinition(root, "Root", "", new Vec3(), new Vec3(0, .1f, 0)),
                new BoneDefinition(child, "Child", root, new Vec3(), new Vec3(0, .1f, 0)),
                new BoneDefinition(leaf, "Leaf", child, new Vec3(), new Vec3(0, .1f, 0)),
                new BoneDefinition(tip, "Tip", leaf, new Vec3(), new Vec3(0, .1f, 0))
            });
            var mesh = AuthoringFixtures.Panel(1);
            var inputs = Enumerable.Range(0, mesh.VertexCount).SelectMany(i => new[]
            {
                new SkinBinding.VertexWeightInput(i, root, .123f),
                new SkinBinding.VertexWeightInput(i, child, .234f),
                new SkinBinding.VertexWeightInput(i, leaf, .345f),
                new SkinBinding.VertexWeightInput(i, tip, .456f)
            });
            var binding = SkinBinding.Create(mesh, skeleton, inputs);
            var bytes = RigCodec.WriteBinding(binding);
            var restored = RigCodec.ReadBindingUnbound(bytes);
            True(bytes.SequenceEqual(RigCodec.WriteBinding(restored)));
        });
    }
}
