...
更新: 2026-09-12。I04-Aの数値基盤、node読取・階層合成、一般inverse-bindを保持するSourceSkin候補型を追加。次はGLBのmatrix accessor読取と保存への接続。VRM0/1の対応profileはGUI接続とPlayer handler検証済み。直近Core369件合格、任意の実モデル取込・実操作・性能の受入は未完了。
...
|---|---|---|
| [ ] I04-A / P1 **次に実装** | source local/world、一般TRS/matrix、inverse-bind、法線/接線変換の基盤 | 回転/scale/鏡映/階層/元原点とbindの差の数値回帰、旧translation/native移行契約。SourceAffine、node decode/階層合成、SourceSkin候補型は追加済み。次はmatrix accessor decodeと保存への接続 |
