...
- 標準SkinnedGeometry GLB出力はmesh nodeの `matrix` としてaffineを保持する。Core **428 passed / 0 failed**（`dotnet run --project Tests/Authoring.Core/Authoring.Core.Tests.csproj --no-restore`、artifact `C:/Users/tomoaki/AppData/Local/Temp/NyaForge-Core-Tests-2ad230ba6a3245f6a9e5eb69ad5bafa3`）。v6 codec、表示投影、matrix出力を合成fixtureで確認した。
