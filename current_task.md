...
同じprivate projectで低ポリカフ（136頂点）を生成し、skinned GLB出力→`skinned-clothing-v1` package化→実RadDollV3 FBX prefab instanceへの`SkinnedClothingReceiver.ApplyPackage`まで通した。`lower_arm.L`を明示BoneId mapへ割り当て、生成Rendererの親、rootBone、頂点数、ownership markerを確認した。さらに同じpackageを再読込して再適用し、管理対象Renderer数を21（元の20＋衣装1）のまま維持、bindingのObjectId／生成object参照を更新できた。証拠は`private/PhysBonesSdkProbe-20260914/avatar-clothing-reapply-report.json`。これは実FBX階層への初回・再適用smokeであり、native保存／再読込、EditorWindowの手動操作、fit・貫通、見た目、Build & Test、実VRChat内受入は未完了である。
...
| 2 | NF-V1-03A | 実装・合成Bridge受入済み / 実FBX初回・再適用smoke済み | private RadDollV3 FBXへカフpackageを`lower_arm.L`へ明示適用し、136頂点・rootBone・ownership marker、同一package再適用時の重複なしを確認。次はnative保存／再読込と実EditorWindow経路を確認 |
