...
[揺れ・布adapter計画](docs/Secondary-Motion-Plan.md)へ採用方針・依存・完了条件を整理した。**PhysBones優先、MagicaCloth2は任意adapter**。SIM-01のUnity非依存契約とcodec、SIM-02のPhysBones target DTO／loss report境界、schema 4 attachment保存、Workbench状態表示を実装したが、PhysBones／MagicaCloth2の導入・実行接続はまだ行っていない。直近はSIM-02のUnity SDK BridgeとI04-Aの保存接続を継続する。
