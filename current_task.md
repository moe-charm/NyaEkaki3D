...
ChatGPT Proの持込Windows v1案を現行 `41e73ac` に照合し、[採用修正版](docs/Windows-v1-Development-Plan.md)へタスク化した。製品全体のC0〜C5と進行中goalは維持する。以降の着手順はこの欄と採用修正版を優先し、下に残る日付付き記録の「次」は当時の履歴として読む。
...
| 2 | NF-V1-03A / 02A | 未実装 / 未受入 | 剛体小物・skin衣装を既存Unity avatarへ初回適用。独立reader/実SDK/local VRChatで比較。現行static Bridge成功でskin適用を代用しない |
