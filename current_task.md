...
| [ ] SIM-01B / P1 | native attachmentとVRM0移行 | `secondary-motion.nyaforge.bin` として `NYSM` をschema 4へ接続し、VRM0 source-node subtree→stable BoneId移行、未知wire版の保持・GUI表示、保存後のskeleton/topology stale診断、明示的なbone/vertex rebind API、同一BoneIdのGUI再bindまで実装。残りは実素材確認。SIM-03Aの前提 |
