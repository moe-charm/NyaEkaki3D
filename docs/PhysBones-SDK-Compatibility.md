...
- 実アバター接続probe: `private/PhysBonesSdkProbe-20260914/avatar-physbone-report.json`（`status: passed`）。同じUnity／SDKでRadDollV3 FBXをimportし、`Skirt_B_1_1.L`→`Skirt_B_1_2.L`直下chainへreflection adapterから実`VRCPhysBone`を生成・設定した。これは型・階層・BoneId接続の確認で、揺れの見た目、衣装適用、Build & Test、実VRChat受入は含まない。
