...
- `forge_surface_fit_inspect`を追加した。NyaForge画面で選択中の衣装graph、avatar対象、avatar面領域、衣装頂点領域、fit offset、最大距離をそのまま使い、`MeshSurfaceFit`の評価頂点数・移動候補数・投影距離・移動量をJSONで返す。さらに最近面のwindingに対するsigned clearanceをサンプルし、`behindSurfaceVertexCount`、最小／最大signed距離、最大64件の`behindSurfaceVertexIds`を返す。
...
- clearanceは裏側へ入った頂点の候補値で、三角形交差・閉じた体積判定・貫通ゼロを保証しない。これは候補形状の数値確認であり、見た目・実VRChat受入を意味しない。対象選択や実操作の受入は手動ゲートとして分離する。
...
`get_state`には、制作対象パネルで参照保護したobject IDを`referenceProtectedObjectIds`として返し、active objectが保護中なら`activeObjectReferenceProtected: true`になる。納品対象を明示したID集合は`deliveryAllowlistObjectIds`、明示モードは`deliveryAllowlistExplicit`として返す。保護中の編集要求は`REFERENCE_PROTECTED`で停止し、object選択・history・保存は継続できる。汎用GLB／multi-object exportはallowlistを使い、保護bodyが対象に残る場合は`REFERENCE_EXPORT_BLOCKED`で停止するため、AIは参照bodyを避けて衣装objectを選択してから出力できる。空projectのnamed pipe stateで、これらの項目が常に返ることをPlayer回帰する。
