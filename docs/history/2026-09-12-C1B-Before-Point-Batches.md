...
更新: 2026-09-12。未接続点の移動→面生成→Undo/native/BakeをGUI検証。Windows suite成功。Core変更なし（直近212件）。
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\Tools\Test-NyaForgeAuthoring.ps1 -BuildName Windows-C1B-LooseToFaceLayout -Width 1280 -Height 800 -TimeoutSeconds 600
```
