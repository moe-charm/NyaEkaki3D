...
更新: 2026-09-12。順序付き頂点からの面作成Core/commandを追加、Core210件成功。面作成GUIは次工程。最新PlayerはWeld。
...
- 未実装: GUIで周囲の順番を明示して編集する操作とPlayer検証、新規頂点の配置、空polygonへの最初の面。現在の選択集合を単にID順にして「クリック順」と扱わないこと。GUIは順番が分かる入力と反転操作、作成前previewを検討する。
...
- 今回新規Playerビルドなし。以下のWeld/BoundaryBridgeの確認済み機能を保持。
## 頂点weld: CoreとGUI実装済み
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\Tools\Test-NyaForgeAuthoring.ps1 -BuildName Windows-C1B-Weld -Width 1280 -Height 800 -TimeoutSeconds 600
```
