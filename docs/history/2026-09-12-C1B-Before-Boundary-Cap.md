...
更新: 2026-09-12。UV島の左ドラッグ移動を実装。仮表示→1command確定、Esc/捕捉喪失取消、古いrevision拒否、範囲外pick、保存/BakeをWindows Playerで確認。
...
- `UvPreview.Drag` にpointer gesture・3px閾値・仮表示offset・取消処理を分離。`AuthoringWorkbench.UvDrag` は開始時のcontext/face選択/envelope revisionを捕捉し、離したときに既存UV変形commandを1回実行する。島全体へ選択を展開し仮表示と確定対象を合わせる。仮表示はUV画面だけ。
- Bind/geometry change/blur/detach/Esc/capture loss、wheel/pan/reset/パネル閉じ/Viewerへ戻る操作で取消。capture通知の到着前にUpを受ける経路の不具合をPlayer試験で発見し、Move/Up時にHasPointerCaptureを直接確認することで修正。
- 最終Player: `Builds/Windows-C1B-UvDragCaptureFix/NyaForge.exe`。build log `Logs/build-player-20260912-034944-022.log`。PASS: `Artifacts/Authoring-20260912-035012-2fa20923648f436abae611a20a2ab9cb/report.json`。複数Moveで文書不変、releaseでrevision+1、1Undo/Redo、UV範囲外への移動とpan後pick、Esc/capture取消、stale envelope拒否、native/Bakeを検証。既存suiteも成功。Core本体変更なし（直近188件）。
- 失敗再現: `Artifacts/Authoring-20260912-034905-59cf5ca487584003a36f6b789ab1d755/report.json` のcapture-loss failure。旧 `Windows-C1B-UvDrag` はこの修正前ビルドなので利用入口は上記CaptureFix。
- 未確認: 複数島の実ドラッグ、Paint付き作品、OS/DPI/focus離脱、project切替の実操作、回転/拡縮ハンドル。全体完成ではない。前段の記録: [履歴](docs/history/2026-09-12-C1B-Before-Uv-Drag.md)。以下は以前の検証結果。
...
1. 造形の面作成/境界を閉じる操作を調べ、既存polygon ID/UV属性と共通commandへ接続する。複数島/拡縮後のドラッグ、Paint付き作品の動作とUV回転/拡縮ハンドルも維持する。merge/bridge/cut、空polygonと削除後のID割当は残件。scene override、GUI実操作、staging登録前中断/未確認新規GUIDなども保持。復旧だけの類似テストを増やし続けない。複数材質の同UUID共有receiver fixture、部位別GPU定量/alpha、mask/重なり遮蔽は検証残件として保持。
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\Tools\Test-NyaForgeAuthoring.ps1 -BuildName Windows-C1B-UvDragCaptureFix -Width 1280 -Height 800 -TimeoutSeconds 240
```
