...
更新: 2026-09-11。3D描きかけの保存/出力隔離、対象切替・カメラ操作による取消を追加検証。カメラ全体表示とmask筆変更の取消漏れを修正。Core143件の既存結果、最新Player合格。製品全体の開発は継続中。
...
- 最新ビルド `Builds/Windows-C1C-SurfaceLifetime/NyaForge.exe`。成功ログ `Logs/build-player-20260911-230207-545.log`。
- 最新Player合格 `Artifacts/Authoring-20260911-230303-c321c5c9a7a34feeacaf15a3f70a3a7e/report.json`。`surface-pending-save` / `surface-pending-png` / `surface-pending-export` は描きかけ中の出力証拠。保存/出力はpointer capture中のAPI操作として検証し、同時にGUIボタンをクリックできたとは主張しない。
...
1. [3Dペイント開発契約](docs/Surface-Paint-Development.md)に沿い、複数面・UV seam・背景・範囲外UVの手前の面を横切るgestureをPlayerで確認。Alt回転/右移動、対象変更、描きかけの保存/出力は今回確認済み。viewport寸法変更の専用試験と、細い面/遮蔽物の精度は残件。2px sampleだけでは十分でなく、適応的sample/edge交差を精密化する。非隣接面へのsample飛びは現状保守的に区間を切る。
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\Tools\Test-NyaForgeAuthoring.ps1 -BuildName Windows-C1C-SurfaceLifetime -Width 1280 -Height 800
```
