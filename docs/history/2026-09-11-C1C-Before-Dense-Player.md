...
更新: 2026-09-11。探索raw UVと保存筆跡を分離し、誤差を制御した簡略化を追加。13万三角形の800px直線がCore計測で成功。Core153件とPlayer合格。製品全体の開発は継続中。
...
- `PaintPathSimplifier` を独立Coreモジュールとして追加。最大4096 raw UV点を区間ごとにDouglas-Peucker法で簡略化し、従来の1024点commandへ渡す。UV誤差は `1/(8*1024)` 以下。端点・区間を保持し、折返しや複雑な線を無理に潰さない。予算超過時はsampler全体をinvalid化する。
- `SurfaceStrokeSampler.PointCount` はraw UV数を保持し、`Snapshot().PointCount` が保存する筆跡の点数。新しいraw点がないsnapshotは再利用。2D path APIと既存保存画像は変更しない。
- Core **153 passed / 0 failed**: `C:/Users/tomoaki/AppData/Local/Temp/NyaForge-Core-Tests-1b1a2cb7ceb94e4093f923a4677932c3`。3000点直線、折返し、区間保持、曲線の幾何誤差/画素比較、raw/final予算、失敗後のinvalid化と旧snapshot保持を検証。
- 修正後profile `C:/Users/tomoaki/AppData/Local/Temp/NyaForge-Core-Tests-b49386d1c0fb46d688ee711a0c85773e/surface-profile.json`。2,048〜131,072三角形すべて800px直線が成功。最大ケースは3576ray / 3574 raw UV / 保存2点。探索から最終snapshotまで約9.61msで、Unity描画/合成時間は含まない。[比較資料](docs/Surface-Paint-Performance.md)参照。
- 最新ビルド `Builds/Windows-C1C-StrokeReduction/NyaForge.exe`。成功ログ `Logs/build-player-20260911-234223-056.log`。
- 最新Player合格 `Artifacts/Authoring-20260911-234245-5baf1a364d824207a8df80784e91c949/report.json`。既存GUI色/mask・4種の境界・viewport resize・取消・Undo・native/Surfaceの回帰を確認。高密度長尺strokeの時間はCore測定のみで、Player性能受入は別途必要。
- 変更前の記録は [履歴](docs/history/2026-09-11-C1C-Before-Stroke-Reduction.md)。commit/pushなし。

## 前段: viewport resizeと投影cache

- `SurfaceViewportVerification` を分離して追加。描きかけ中に実layoutでviewportを幅/高さ75%へ変更。取消・pointer解放・render target交換、遅いpointer upの無効化、新しい座標でUV(.85,.75)へ描画・1command・Undo、元サイズへの復帰を確認。
- `SurfacePaintProfile` をCore試験projectの明示 `--surface-profile` オプションに追加。合成格子2,048〜131,072三角形の構築/描画時間とsample数を記録。[計測資料](docs/Surface-Paint-Performance.md)に値と限界を記載。
- `SurfaceProjectionCache` は1snapshotを保持し、同じ形状/配置/camera行列/viewport/near/farなら再利用。workspace交換時はClearする。Playerで同条件の再利用とresize時の交換を確認。131,072三角形で約270msだったcoverageの構築を連続strokeごとに繰り返す問題へ対処。Unityでの速度改善率は未計測。
- 当時のビルド `Builds/Windows-C1C-ProjectionCache/NyaForge.exe`。成功ログ `Logs/build-player-20260911-233542-561.log`。
- 当時のPlayer合格 `Artifacts/Authoring-20260911-233611-a42e99aafc1c466d80ffc6fd48d2571e/report.json`。resize検証は3D paint GUI試験内で実行。cache導入前のresize単独合格は `Artifacts/Authoring-20260911-233328-955b8527acce4653af93d460524e8609/report.json`。
- Core実装変更なし。Core149件は前段の既存結果。今回のCore profileは計測でありテスト合格件数へ加えない。8,192三角形以上の800px strokeでUV1024点上限による失敗を確認し、次の改善項目とした。
- 変更前の記録は [履歴](docs/history/2026-09-11-C1C-Before-Projection-Cache.md)。OSウィンドウ枠/DPIの手動操作は未確認。commit/pushなし。
...
- 当時のビルド `Builds/Windows-C1C-ScreenCoverage/NyaForge.exe`。成功ログ `Logs/build-player-20260911-232821-703.log`。
- 当時のPlayer合格 `Artifacts/Authoring-20260911-232854-e9b3879e4e1a4f928b5e4c1198d54249/report.json`。`surface-boundary-thin-occluder` / `-export` / `.png` が追加証拠。既存色/mask・境界・取消・保存/出力の回帰も合格。
...
1. 長いstrokeの簡略化はCore/既存Player回帰まで合格。[負荷計測](docs/Surface-Paint-Performance.md)の高密度ケースをPlayer側でも確認し、texture更新・合成・GCを含む負荷を切り分ける。曲がった長いstroke、多数の区間、ray4096回上限、camera変更後の構築費、退化したscreen交差とOS/DPI手動操作は残る。
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\Tools\Test-NyaForgeAuthoring.ps1 -BuildName Windows-C1C-StrokeReduction -Width 1280 -Height 800
```
