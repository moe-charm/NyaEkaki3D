...
更新: 2026-09-11。複数面・UV seam・背景の隙間・範囲外UVの手前の面を横切る3D GUI試験を追加。不透明previewのalpha漏れを修正し、camera画素と保存画像の両方を確認。最新Player合格。製品全体の開発は継続中。
...
- 最新ビルド `Builds/Windows-C1C-SurfaceBoundariesOpaque/NyaForge.exe`。成功ログ `Logs/build-player-20260911-231541-493.log`。
- 最新Player合格 `Artifacts/Authoring-20260911-231603-25b6c0b288ad41368b6a4ed5338b0b10/report.json`。同ディレクトリの `surface-boundary-{seam,gap,occluder}.png` / native保存フォルダ / `-export` が証拠。
...
1. [3Dペイント開発契約](docs/Surface-Paint-Development.md)に沿い、viewport寸法変更の専用試験と、細い面/遮蔽物の精度を進める。複数面・UV seam・背景・範囲外UVの手前の面を横切るgestureは幅のある合成fixtureでPlayer確認済み。2px sampleだけでは十分でなく、適応的sample/edge交差を精密化する。非隣接面へのsample飛びは現状保守的に区間を切る。
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\Tools\Test-NyaForgeAuthoring.ps1 -BuildName Windows-C1C-SurfaceBoundariesOpaque -Width 1280 -Height 800
```
