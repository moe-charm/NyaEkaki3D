...
更新: 2026-09-12。初回importの所有receiptと読取専用の変更検査を追加。Unity側の保存済み/未保存変更の検出を確認。更新適用は未実装。
...
- `ImportOwnership` を追加。初回importでdocument/object/revision、生成ファイルの相対path・GUID・内容/meta hashを `NyaForgeImport.json` に記録。Inspectorは変更/欠落/dirtyを返し、書込みを行わない。
- Import画面に「既存インポートの確認」フォルダと「生成後の変更を調べる」ボタンを追加。GUI実操作は未検証。更新の適用ボタンはまだない。
- 最終Bridge PASS: `Artifacts/BridgeReceiver-20260912-025112-198-819134b915a443f79ad10a17d9781e6f/bridge-report.json`。Unity2022.3.22f1。所有記録、未保存材質変更、保存済み変更、元bytes復元、不正path拒否と旧各profileのimportを回帰。Core/Playerは今回変更なし、以下は前段証拠。
- [更新契約](docs/Unity-Import-Updates.md)を追加。現Bakeには明示output node UUIDがないため、次は版付き出力identityの追加から進める。Prefabの未管理component保存と実更新transactionは未実装。
- 前段: [履歴](docs/history/2026-09-12-C1C-Before-Import-Ownership.md)。
...
1. `docs/Unity-Import-Updates.md` に沿い、版付きoutput identity→更新候補/競合preview→既存GUIDを保持する更新transactionを実装する。所有receiptと変更検査は追加済み。複数材質の同UUID共有receiver fixture、部位別GPU定量/alpha、mask/重なり遮蔽は検証残件として保持。
