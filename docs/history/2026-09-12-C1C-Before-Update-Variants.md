...
更新: 2026-09-12。復旧途中Kill→再起動再試行を確認。資源の追加/更新/保持計画をGUIと適用処理で共用し、新材質/画像の追加とrollbackを確認。
...
- 未確認: 元々1枚を共有する画像の分岐/再共有、幾何/slot数変更の更新、scene override、GUI実操作、GUID記録前中断、staging journal登録前/commit直後Kill、disk障害。今回の追加は材質UUID交換で、全variantの完了ではない。
## 次の作業

1. 共有画像の分岐/再共有と幾何・slot数変更を資源計画に通して検証し、必要な修正を行う。その後はC1の造形/UV機能の残件へ進める。scene override、GUI実操作、staging登録前中断/未確認新規GUIDなどは保持。復旧だけの類似テストを増やし続けない。複数材質の同UUID共有receiver fixture、部位別GPU定量/alpha、mask/重なり遮蔽は検証残件として保持。
