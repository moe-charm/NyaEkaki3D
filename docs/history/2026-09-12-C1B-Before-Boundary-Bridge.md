...
更新: 2026-09-12。辺への頂点追加を実装。共有頂点と面ごとのUV seamを保持し、GUI百分率/新頂点選択/Undo/native/Bakeを確認。Core201件、Player suite成功。
...
- `PolygonEdgeInsertion`/`CornerInterpolation`/`EdgeInsertionOperation`/`AuthoringWorkbench.EdgeInsertion`を分離。選択辺の小ID→大IDを基準に内分点を生成し、1つの頂点を全隣接面へ共有する。各面に新cornerを別々に追加し、UV seamを保持。normalは正規化、tangentは直交化。属性退化/反対handedness/非manifold/端点fractionを拒否。
- 選択頂点のrender index→stable ID変換を `AuthoringWorkbench.PolygonSelection` へ共通化。GUIはfoldout内の百分率とID方向表示。成功後は新しい頂点（seam時は全render alias）を選択。失敗時は文書/選択を保持。
- Core **201 passed / 0 failed**。境界辺25%挿入、既存corner/位置/ID保持、共有辺UV seam、追加点からの面分割、無効指定、Undo/Redo/native/Bakeを検証。結果: `C:/Users/tomoaki/AppData/Local/Temp/NyaForge-Core-Tests-16b26718eaf6460eb1fcaff25ad77b61`。
- 最新Player: `Builds/Windows-C1B-EdgeInsertion/NyaForge.exe`。build log: `Logs/build-player-20260912-042746-703.log`。PASS: `Artifacts/Authoring-20260912-042820-15b1087a40b04c99877b72bfdf4e18a8/report.json`。GUI25%挿入・新点選択・0%拒否時の選択保持・Undo/Redo/native/Bakeと既存suiteを確認。`edge-insert.png`を目視し追加点とGUIを確認（入力欄0は直前の拒否テストで残した値、保存形状は25%位置）。
- 未確認: 複雑なshading補間/縮尺/極端なfraction、Paint付き作品、shared seamのGUI操作。三角分割/補間が変わる可能性あり。既存suiteの所要時間を考慮し今回timeoutは600秒、正常終了済み。前段記録: [履歴](docs/history/2026-09-12-C1B-Before-Edge-Insertion.md)。以下は以前の面分割記録。
...
1. 2つの境界を面でつなぐbridgeを進める。既存境界選択/ハイライトとID割当を再利用し、共通command/Undo/native/GUIへ接続する。自由面作成・vertex weld/多面merge・連続knifeも維持。複数島/拡縮後のドラッグ、Paint付き作品の動作とUV回転/拡縮ハンドルも維持する。merge/bridge/cut、空polygonと削除後のID割当は残件。scene override、GUI実操作、staging登録前中断/未確認新規GUIDなども保持。復旧だけの類似テストを増やし続けない。複数材質の同UUID共有receiver fixture、部位別GPU定量/alpha、mask/重なり遮蔽は検証残件として保持。
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\Tools\Test-NyaForgeAuthoring.ps1 -BuildName Windows-C1B-EdgeInsertion -Width 1280 -Height 800 -TimeoutSeconds 600
```
