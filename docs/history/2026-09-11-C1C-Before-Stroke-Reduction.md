...
更新: 2026-09-11。viewport resizeの取消/再描画をPlayerで検証。負荷計測から投影coverageの再利用を追加。最新Player合格。長いstrokeのUV点数上限が次の改善対象。製品全体の開発は継続中。
...
- 最新ビルド `Builds/Windows-C1C-ProjectionCache/NyaForge.exe`。成功ログ `Logs/build-player-20260911-233542-561.log`。
- 最新Player合格 `Artifacts/Authoring-20260911-233611-a42e99aafc1c466d80ffc6fd48d2571e/report.json`。resize検証は3D paint GUI試験内で実行。cache導入前のresize単独合格は `Artifacts/Authoring-20260911-233328-955b8527acce4653af93d460524e8609/report.json`。
...
1. [負荷計測](docs/Surface-Paint-Performance.md)を根拠に、長いstrokeの探索sampleと保存するUV筆跡を分離し、誤差を制御して点数を削減する。8,192三角形のplaneでも800px strokeが1024点上限に達する現状を改善する。viewport寸法変更の専用Player試験は合格。camera変更後の再構築費、退化したscreen交差、密な実モデルの負荷、OS/DPI手動操作も保持する。
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\Tools\Test-NyaForgeAuthoring.ps1 -BuildName Windows-C1C-ProjectionCache -Width 1280 -Height 800
```
