...
GLB取込結果には `GlbImportDiagnostic` のコード付き診断を付ける。選択meshに材質・画像リソースがある場合は `MATERIALS_NOT_RETAINED`、animationは `ANIMATIONS_NOT_RETAINED`、`extensionsRequired` は `REQUIRED_EXTENSIONS_NOT_RETAINED`（blocking）、`extensionsUsed` は `EXTENSIONS_PARTIAL`（partial）として、source pathと理由を `Diagnostics` に残し、既存の `Warnings` にも同じコードを表示する。これは完全保持を主張しないための入口であり、診断がある入力を依存資源ごとnativeへ保存したり、GUI/MCPで詳細reportを公開したりする実装ではない。
