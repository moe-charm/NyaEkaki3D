...
現行readerには本仕様をまだ満たさない箇所もある。`GlbImport`は静的scene構造・追加UV・外部URI画像等を保持しない。選択primitiveの基本PBR係数と埋め込みbase color PNG/JPEGはnative material graphへ渡すが、その他のtexture/image/samplerと追加材質拡張は保持せず診断する。NORMAL/TANGENT morphはbase属性が揃う場合に保持し、欠ける場合はPOSITIONを残してwarningを返す。`GlbSceneInventoryReader`は複数mesh/instance/skinの参照とnode world transformを候補化し、`GlbImporter.Read(bytes, meshIndex)` / `GlbSkinImporter.Read(bytes, meshIndex, skinIndex)` / `GlbSourceSkinImporter.Read(bytes, meshIndex, skinIndex)` とWorkbenchの選択GUIは選択した候補を一つずつnative objectへ公開できる。16bit JOINTSはbyte幅を保って読取り、source skin表示は最終graph出力へ適用して下流編集を保持する。Workbenchのactive object編集と非active object同時表示は実装済みだが、結合出力、同名morph/共有参照は未実装。未知の `extensionsRequired` を網羅して拒否する処理、任意のVRM meta/利用条件/未知拡張を依存込みで保管する機構も未実装。これらはI04-B/Eの解消対象で、現在の取込成功を本仕様の完全保持成功と称しない。容量監査には16MiB/blob・200,000頂点・32submesh等の予算も含める。

GLB取込結果には `GlbImportDiagnostic` のコード付き診断を付ける。選択meshに材質・画像リソースがある場合は `MATERIALS_NOT_RETAINED`、animationは `ANIMATIONS_NOT_RETAINED`、`extensionsRequired` は `REQUIRED_EXTENSIONS_NOT_RETAINED`（blocking）、`extensionsUsed` は `EXTENSIONS_PARTIAL`（partial）として、source pathと理由を `Diagnostics` に残し、既存の `Warnings` にも同じコードを表示する。基本PBR係数と埋め込みbase color画像をnative material graphへ渡しても、その他のtexture/image/sampler等が欠けるため診断は残す。これは完全保持を主張しないための入口であり、`import-diagnostics.nyaforge.json`へgraphId単位でsnapshot保存し、`forge_graph_inspect`、取込後status、GLB取込パネルのGUI詳細reportへ公開する。診断がある入力を材質・animation・未知拡張の依存資源ごとnativeへ保存する実装、required拡張の網羅的拒否は未完了。
