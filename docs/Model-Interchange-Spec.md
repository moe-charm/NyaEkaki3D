...
GLB取込結果には `GlbImportDiagnostic` のコード付き診断を付ける。選択meshに材質・画像リソースがある場合は `MATERIALS_NOT_RETAINED`、animationは `ANIMATIONS_NOT_RETAINED`、`extensionsRequired` は `REQUIRED_EXTENSIONS_NOT_RETAINED`（blocking）、`extensionsUsed` は `EXTENSIONS_PARTIAL`（partial）として、source pathと理由を `Diagnostics` に残し、既存の `Warnings` にも同じコードを表示する。これは完全保持を主張しないための入口であり、`import-diagnostics.nyaforge.json`へgraphId単位でsnapshot保存し、`forge_graph_inspect`と取込後statusへ公開する。診断がある入力を材質・animation・未知拡張の依存資源ごとnativeへ保存する実装、GUIの詳細reportパネル、required拡張の網羅的拒否は未完了。
