...
`current.StandaloneWindows64.json`はViewerの確認パックを選ぶためのpointerであり、Authoringのnative正本ではない。Authoringの正本は`project.nyaforge.json`と`blobs/`および許可されたversioned attachmentである。

コード照合対象は現行main。GLB importは一回の制作操作で一つの明示候補をgraph objectとして公開し、一般affine（node TRS/matrix・inverse-bind）、32weight、512骨/512morphの範囲で扱う。WEIGHTSのfloat形式に加え、normalized UBYTE/USHORTはfloatへ復元してから同じbinding検証へ通す。sparse accessorは引き続き未対応として拒否する。`GlbSceneInventoryReader`は複数mesh/instance/skinを元indexで保持し、Workbenchはnode instance indexを指定した場合にそのnodeのmesh／skin対応を採用する。静的meshでは選択nodeのworld affineをpositions、normal、tangent、POSITION/NORMAL/TANGENT morphへ適用する。skinを持たないnode instanceは静的meshとして扱い、GLB全体に別skinがあることだけを理由にskinned importへ回さない。通常の単体取込に加え、Workbenchの「全mesh instanceを取り込む」は同一GLB/VRMのbody・hair等を各node instanceのaffine付きgraph objectへ展開し、全候補の検査が完了してから一つのcommand batchで原子的に公開する。途中の候補で失敗した場合は文書・metadata・Undoを変更せず、object上限64件を超える場合は事前に拒否する。native documentは最大64 objectのactive object方式で、`object.select`とSave/Openを提供する。 skinned objectのsource rig sessionはGraphIdで分離した`imported-rig-sessions.nyaforge.bin`へ保存し、active object切替時に対応sessionを選ぶ。取込source nodeは最大4096、一時実行骨は最大512。複数objectは`multi-object.nyaforge-bake.json`配下へobjectごとのBakeを束ねて出力できるが、mesh結合・共有参照は未実装。同一skeleton／poseを共有する複数SkinDeformの同時評価は実装済み。nativeの材質/rig機能が進んでいても、任意の外部モデルを完全に取り込める段階ではない。 rig/morphを含むgraphの出力は`project.nyaforge.json` native packageへルーティングし、Skin/Morph情報を保持したまま再開できる。初期VRM1 humanoid/meta profileは実装済みだが、完全VRM出力と受取先別のskin/morph互換検証は未完了である。

現行readerには本仕様をまだ満たさない箇所もある。`GlbImport`は静的scene構造・追加UVを保持しない。選択primitiveの基本PBR係数と埋め込みまたは安全なローカル相対URIによるbase color PNG/JPEGはnative material graphへ渡し、1024pxを超えるbase colorは最大8192pxまでデコードしてアスペクト比を維持した縮小画像をnative Paintへ所有保存する。8192px超、壊れた画像、16MiB超、remote/data URI、モデルフォルダ外の相対URI、base-color以外のtexture/image/samplerと追加材質拡張は保持せず診断する。`GlbSceneInventoryReader`は複数mesh/instance/skinの参照とnode world transformを候補化し、`GlbImporter.Read(bytes, meshIndex)` / `GlbSkinImporter.Read(bytes, meshIndex, skinIndex)` / `GlbSourceSkinImporter.Read(bytes, meshIndex, skinIndex)` とWorkbenchの選択GUIは選択した候補を一つずつnative objectへ公開できる。16bit JOINTSはbyte幅を保って読取り、source skin表示は最終graph出力へ適用して下流編集を保持する。Workbenchのactive object編集と非active object同時表示は実装済みだが、結合出力、同名morph/共有参照は未実装。`extensionsRequired` に未対応項目がある入力は取込前に `UNSUPPORTED_EXTENSION` で拒否する。任意のVRM meta/利用条件/未知拡張を依存込みで保管する機構は未実装で、任意入力を完全保持した成功とは称さない。容量監査には16MiB/blob・200,000頂点・32submesh等の予算も含める。

GLB取込結果には `GlbImportDiagnostic` のコード付き診断を付ける。選択meshに材質・画像リソースがある場合は `MATERIALS_NOT_RETAINED`、animationは `ANIMATIONS_NOT_RETAINED`、`extensionsUsed` は `EXTENSIONS_PARTIAL`（partial）として、source pathと理由を `Diagnostics` に残し、既存の `Warnings` にも同じコードを表示する。`extensionsRequired` に未対応項目がある入力は診断を公開する前に `UNSUPPORTED_EXTENSION`（blocking）で停止する。基本PBR係数と埋め込みbase color画像をnative material graphへ渡しても、その他のtexture/image/sampler等が欠けるため診断は残す。これは完全保持を主張しないための入口であり、`import-diagnostics.nyaforge.json`へgraphId単位でsource hash・mesh index・skin index・明示node instanceのnode indexとともにsnapshot保存し、`forge_graph_inspect`、取込後status、GLB取込パネルのGUI詳細reportへ公開する。診断がある入力を材質・animation・未知拡張の依存資源ごとnativeへ保存する実装は未完了。容量監査には16MiB/blob・200,000頂点・32submesh等の予算も含める。
...
| 2 / I04-B | `GlbSceneInventoryReader`で複数mesh/instance/skinの候補とsource mappingを保持し、active object方式で候補を複数制作対象へ公開、選択・保存/Openを検証済み。次は同時表示・結合出力、同名target・共有参照を検証 |
...
| profile | 保持する情報 | 境界 |
|---|---|---|
| `StaticGeometry` | 現在評価できる表示メッシュ、頂点属性、正の一様変換、標準PBR材質と埋め込みbase-color PNG | skin・骨・morph・アニメーションは含めず、未対応のtexture/image/samplerは出力しない。材質slotがない旧形式は複数submeshを一つのprimitiveへ結合 |
