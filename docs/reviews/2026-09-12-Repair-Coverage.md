...
| R02 collider node列 | VrmMetadataTests: VRM0/1の取込・codec往復、legacy native往復。Player SaveFailureVerification: 重複/混在node列のWorkbench保存・Open | 同じfixtureでVRMファイル取込からWorkbench Openまで一気通貫の試験は未実施 |
...
| R10 検証強化 | 各修正に再現を追加。null group拒否、実参照ありの衝突、前stateを引き継ぐ反復を検査 | R02一気通貫と、上記手動/外部transport受入を分けて記録する |
...
R10は上記の検証境界を明記したうえで、R02の同一fixtureによる取込→Workbench Save/Openまで完了させる。手動受入は独立タスクとして維持する。C0〜C5の製品目標を、この修正レビューの終了へ縮小しない。
