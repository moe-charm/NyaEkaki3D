...
| NF-V1-06 | 範囲を指定するfit/weight | 03の座標/対象契約 | 衣装の選択頂点、元body面領域、距離を共通候補として使う。Workbenchのavatar面ID・衣装頂点ID入力とビューポート選択の取込をfit／weightへ接続済み。範囲外は無変更、未選択頂点・weightを保持。薄い表裏・袖/胴体・遠方の負例。候補/未対応点/移動量を確認して確定 |
