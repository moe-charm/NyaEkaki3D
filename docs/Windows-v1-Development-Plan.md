...
## 2026-09-14 優先順位の更新: GUI導線とソースの責務分割

2026-09-15追記（source commit `c778896`）: VRM取込で検出したLookAt／FirstPerson／表情material bind（VRM 0.xはmaterialValues）をsemantic inventoryとして記録し、初期profileで保持しないパスを`VRM_SEMANTICS_NOT_RETAINED`のblocking診断としてnative Save/Open・inspection・出力reportへ引き継ぐ経路を追加した。partial出力は従来どおり可能だが、完全意味情報を必要とするstrict判定では停止できる。元payloadの依存込み保管・変換、実EditorWindow／実SDK／実VRChatの受入は未完了として残す。Core **515 passed / 0 failed**、private RadDollV3 VRMのimport-only／VRM-output-only Player smokeを確認済み（詳細は[current_task](../current_task.md)冒頭）。

ユーザーの実操作フィードバックを受け、NF-V1-08の実制作一周／NF-V1-15の手動受入の前提に[GUI導線・モジュール化計画](Windows-v1-GUI-Navigation-Plan.md)のGUI-01〜08、MOD-01〜05を追加する。設計・タスク化に続き、常設コマンドバー、基本形状パネル、対象役割名、取込手順説明、開発者fixtureの分離、保存／出力Panel、MCPPanel、作業モード導線を実装した。V11では作業モード4ボタンのポインタ回帰も追加し、V12では`WorkbenchSession`／`SelectionContext`の第一段を接続した。V17ではRefreshPipelineの更新群分離と、作業モード検証のScrollView安定化を実施し、V18では共通UI生成部品をUiElementsへ分離、V19ではビューポートのFrame／カメラRenderTextureをViewportへ分離した。V20〜V24ではstable ObjectId keyed表示名metadataを`ObjectLabelsCodec`へ分離し、対象一覧・native Save/Open・Undo/Redo・inspectionを接続、Core 510件とPlayer Authoring／Navigationを再確認した。V25〜V26では`ObjectLabelService`／`forge_set_object_label`を追加し、revision・attachment hashを検査するGUI／外部MCP共通経路、Core 512件、MCP SDKビルド、Player Authoring／Navigation／外部sidecar transportを再確認した。V27ではviewport入力（対象選択・頂点hit test・座標変換）を`ViewportInput`へ分離し、V28ではUI Toolkitイベント配線を`ViewportInteraction`へ分離してPlayer Authoring／Navigation回帰を再確認した。V29ではルート／ヘッダー／viewport／controls配置とfeature `Build*`呼出し順を`Layout`へ分離し、同回帰を再確認した。空状態とViewerからの移動、名前の永続化、作業別設定を段階的に整理する。同時にWorkbenchの共有fieldに依存するpartialを、責務のあるパネルと共通状態／操作へ段階的に分ける。現在の責務と次段の移行範囲は[Authoringコードマップ](Authoring-Code-Map.md)に固定する。既存Coreとcommandは再利用する。

V30〜V31では頂点選択変更の部分refresh境界を`AuthoringWorkbench.SelectionRefresh.cs`へ分離した。面編集・Rig・装着表示など選択に依存する範囲だけを更新し、材質・出力・シミュレーションPanelの完全refreshを避ける。V31のPlayer Authoring／Navigation回帰で、face split／edge insertion／weldを含む既存操作の継続を確認した。V32では`AuthoringWorkbench.Objects.cs`に対象一覧の差分更新キーを追加し、頂点編集時に対象ボタンを再生成せず、対象ID・表示名・active objectの変更時だけ一覧を更新する経路を確認した。V34では全体refreshを`RefreshState`へ、V35では共通command実行・例外捕捉・ステータス表示を`Execution`へ、V36では共有UIハンドル・session・selection context・viewport状態を`State`へ、V37では保存状態の表示更新を`PersistenceRefresh`へ分離し、Player Authoring／NavigationとCore回帰を再確認した。V37のprivate RadDollV3自動衣装一周とUnity Bridge受け取りも再確認した。
...
最新追記（2026-09-15、source commit `11449cf`）: private RadDollV3 VRMを入力に、既存のチョーカー生成・頂点1点の編集・Neckへの明示skin-bind・native Save/Open・衣装package出力をWindows Playerで95 checks PASSし、同packageをUnity 2022.3.22f1 Bridgeへ渡す検証もPASSした。保存欄にはnative正本 `project.nyaforge.json` のフルパス、出力欄には汎用納品対象の状態を表示する導線を追加した。さらに共通UIのボタンを狭幅・高DPIでも縮小／折返し可能にし、MCP接続Panelの説明欄も親幅へ追従させた。隔離PlayerのAuthoring 88 checks／Navigation回帰とcaptureで確認している。さらに装着Panelへfit／weight対象サマリーを追加し、衣装頂点・avatar面の範囲、全体数、skin-bind状態、測定の有効性を表示する。作業モード切替時は選択した編集領域だけを開き、前の長いPanelを自動的に折りたたむ。Morph／表情差分は確認・出力モードから開け、選択中targetの完全IDと現在の制作対象名・object IDをtooltip／状態欄で照合できる。Morph target tooltipの完全ID回帰を追加し、隔離Player `NyaForge-MorphTargetContextV1`の通常Authoringは88 checks PASS。source-skin表示・投影キャッシュは複合キーの値比較と割当なし候補走査へ移行し、毎回のキー文字列連結・LINQ列挙を避ける軽量化を追加した。装着Panelのfitサマリーも、対象avatarの不変GraphEvaluationをObjectIdとGraph参照で再利用し、頂点選択ごとの再評価を避ける。空project・装着先解除時は保持評価も解放する。装着Panelの面表示・fit／weight・pose操作も同じavatar評価を共有する。実EditorWindowでの全周fit・貫通・見た目、実SDK／VRChat Build & Test・実機表示は引き続き外部受入として残す。軽量化後のprivate RadDollV3自動一周はPlayer 97 checks、Unity Bridge 16 checksで再確認し、詳細証跡をcurrent_taskへ記録した。検証 harness はUnity起動時に掃除されるproject `Temp`をPhysBones fixtureの出力先に使わない。 装着Panelの候補表示はsource commit `9f7b71e`で表示名・役割・短縮IDへ改め、選択中avatarの完全ObjectIdとBoneのstable BoneIdをtooltipで照合できるようにした。Authoring、private RadDollV3一周、Unity Bridgeの再確認結果は[current_task](../current_task.md) GUI-13へ記録している。
...
本書は持込「NyaForge Windows v1 開発計画」をコード照合して修正した実行計画。受入済み報告ではない。製品全体の目標・C0〜C5の要件は[設計v2](NyaForge-Authoring-Design2.md)を維持し、本書はWindows衣装制作v1へ至る着手順を定める。v1だけの合格を製品全体や進行中goalの完了へ読み替えない。直近の状態は[current_task](../current_task.md)。現行mainではCore 509 passed / 0 failed、`Builds/BoneSubsetV16/NyaForge.exe`のクイックモデル取込／Authoring、実RadDollV3衣装package／Unity 2022.3.22f1 Bridge受け取り、private RadDollV3全mesh import／Save/Open／GLB・VRM1 smoke、実SDKのRadDollV3 Skirt chain PhysBone設定probe、実FBXへのskinned-clothing package初回・再適用・native roundtrip smoke、実body meshを使ったsurface fit／weight transfer接続probe、semantic textureを含む合成Unity Bridgeの衣装package回帰、明示納品対象allowlistのGUI／MCP／GLB subsetとSave/Open回帰まで確認済みで、通常GLBのTEXCOORD_1は`UNSUPPORTED_UV_SET`で明示停止する。semantic textureのGUIもUV1新規適用を拒否し、UV0へ戻してから保存・出力する契約を持つ。base-colorを1024pxへ縮小した場合は原画像サイズと作業画像サイズを表示し、PNG/JPEGの原画像bytesを`image.original-source`としてnative graphへ別保持する。未編集時は原画像bytesをGLBへ再利用し、Paint編集後やsource不在時はbounded previewへ戻す。原画像sourceを持たない既存projectから復元を装わない。範囲限定fitの平均値は選択頂点数を分母にし、`EvaluatedVertexCount`として検査記録へ出す。通常候補の検証一周はworking set約2.73GB／private bytes約3.40GB、全mesh一括は約3.67GB／4.49GBを観測しており、いずれも実EditorWindowアイドル時の予算ではなく、出力・再読込を含む一時ピークとしてNF-V1-15で分離計測する。実マウス・実EditorWindow・全周fit／貫通・見た目・Build & Test・実VRChatは未受入である。

実RadDollV3 VRMの取込・編集・保存・GLB/VRM出力と、制御fixture衣装のpackage生成・Unity Bridge受け取りをまとめて確認する再実行入口は `Tools/Test-NyaForgeRealClothing.ps1`。V26でもprivate `RadDollV3_VRM.vrm`を使った取込→全mesh→Save/Open→GLB／VRM→衣装package→Unity 2022.3.22f1 Bridge（2 bones）をPASSし、証跡は[current_task](../current_task.md)冒頭へ記録した。全mesh取込で一時約4.6GBのPrivate Memoryを観測したため、NF-V1-15では全mesh取込と通常編集を分けて計測する。package出力自体の証拠はcurrent_taskへ分けて記録し、実RadDollV3へ新規衣装を全周fitして見た目と貫通を受け入れる作業は別工程として残す。V24の狭い実ウィンドウ操作折り返し証跡は[current_task](../current_task.md)のNF-V1-09Hへ記録した。手動工程は [Windows v1 手動受入チェック](Windows-v1-Manual-Acceptance.md) に固定する。
...
持込の「P95入力100ms、1080pで30fps、2時間、50回Open/Close、warm baseline比10%または100MiB以内」は暫定測定案。V30では標準fixtureのforeground再生60秒＋停止5秒を実行し、**60.0016 FPS、P95 16.8724ms、最大17.7144ms、private bytes約577MB→563MB、working set peak約335MB**を観測した（レポート `C:/Users/tomoaki/AppData/Local/Temp/NyaForge-PerformanceV30-Foreground-e8eaeb64df894b969e80fee4a092b347/performance.json`）。実RadDollV3では、候補取込直後だけを測る`Tools/Test-NyaForgeAuthoring.ps1 -ImportOnly`と、GLB出力だけを測る`-GlbExportOnly`を追加した。ImportOnlyはworking set peak **1,081.3MB**／private bytes **1,527.8MB**、GlbExportOnlyは**1,668.1MB／2,286.6MB**（出力専用probe、約22.3秒）、BIN長取得の一時コピー削減後は**1,653.7MB／2,238.2MB**（同条件再計測）、readback共有後は**1,669.5MB／2,156.4MB**、VRM1のみの`-VrmExportOnly`は**2,428.9MB／3,145.5MB**（約40.5秒）、コンテナ直接構築後は**2,492.0MB／3,205.3MB**（同条件再計測、改善量未確定）、VRM1出力を除いた通常一周は**2,160.7MB／2,689.4MB**、VRM1出力を含む通常一周は**2,730.7MB／3,399.6MB**、全mesh一括は**3,667MB／4,492.3MB**を観測した（詳細は[current_task](../current_task.md) NF-V1-15A／15C〜15E）。これらはUnity 6000.4.3f1／RTX 4090／Windows 11の単一環境で、出力・再読込を含む一時ピークを含むため、日常アイドル時の予算合否や別環境の保証ではない。V39では50回の自動起動・終了回帰を完了した（`Artifacts/Navigation-Repeated-PerformanceV39-official-50.json`）。軽量化後の隔離Playerでも公開fixture一式を使った20回Navigationを20/20 PASSで再確認した。ただし20回更新・2時間編集、実マウス操作、別Windows環境の最終計測は未実施。FPS平均だけで長い入力停止を見逃さず、保持Unity Object数とmanaged/nativeメモリも記録する。異なる場面のピーク同士を比較してリーク判定しない。
