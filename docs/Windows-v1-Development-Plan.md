...
最新追記（2026-09-15、main `c6da552`）: private RadDollV3 VRMを入力に、既存のチョーカー生成・頂点1点の編集・Neckへの明示skin-bind・native Save/Open・衣装package出力をWindows Playerで95 checks PASSし、同packageをUnity 2022.3.22f1 Bridgeへ渡す検証もPASSした。保存欄にはnative正本 `project.nyaforge.json` のフルパス、出力欄には汎用納品対象の状態を表示する導線を追加した。実EditorWindowでの全周fit・貫通・見た目、実SDK／VRChat Build & Test・実機表示は引き続き外部受入として残す。検証 harness はUnity起動時に掃除されるproject `Temp`をPhysBones fixtureの出力先に使わない。
