...
| SIM-02 / P1・C2 **実装継続** | PhysBones target DTOとUnity Bridge | DTO/`NYPP` v1 codecとschema 4 attachment、target package（manifest/profile/skeleton）、stable bone対応、root/末端/除外/分岐、collider参照、制限、曲線、interaction設定を対象SDK版付きで保持。`PhysBonesLossReport`で対応/未対応/SDK差異をBridge書込み前に明示し、Workbenchで対応版/stale/未知版を表示する。reflection backendと`CreateOrUpdateManaged`／`UpdateManagedOnly`を合成receiverで検証済み。残りは実SDK型でのmapping、受け取り側設定と動作、実アバター／VRChat内確認 |
