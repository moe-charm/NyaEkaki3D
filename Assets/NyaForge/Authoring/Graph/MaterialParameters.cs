...
namespace NyaForge.Authoring.Graph
{
    ...
    public sealed class MaterialParameters
    {
        ...
        /// <summary>Optional semantic normal and metallic-roughness images.</summary>
        public MaterialTextureSet Textures { get; }
        ...
        public MaterialParameters(Vec4 baseColor,float metallic,float roughness,Vec3 emission,
            ...
            : this(baseColor, metallic, roughness, emission, alphaMode, alphaCutoff, null)
        { }
        public MaterialParameters(Vec4 baseColor,float metallic,float roughness,Vec3 emission,
            MaterialAlphaMode alphaMode,float alphaCutoff,MaterialTextureSet textures)
        {
            ...
            BaseColor=baseColor;Metallic=metallic;Roughness=roughness;Emission=emission;AlphaMode=alphaMode;AlphaCutoff=alphaCutoff;Textures=textures;
            using(var stream=new MemoryStream()) using(var writer=new BinaryWriter(stream,Encoding.UTF8))
            { Write(writer); if(Textures != null && !Textures.IsEmpty) { writer.Write(true); writer.Write(Textures.ContentHash); } ContentHash=Checks.Hash(stream.ToArray()); }
        }
        ...
        internal static MaterialParameters Read(BinaryReader reader,MaterialTextureSet textures)=>new MaterialParameters(
            new Vec4(reader.ReadSingle(),reader.ReadSingle(),reader.ReadSingle(),reader.ReadSingle()),reader.ReadSingle(),reader.ReadSingle(),
            MeshBinary.ReadVector(reader),(MaterialAlphaMode)reader.ReadInt32(),reader.ReadSingle(),textures);
    }
}
