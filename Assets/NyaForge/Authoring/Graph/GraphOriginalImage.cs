...
namespace NyaForge.Authoring.Graph
{
    ...
    public sealed class GraphOriginalImage
    {
        ...
        public GraphOriginalImage(string paintNodeId, int width, int height, string mimeType, byte[] encodedBytes, string previewImageHash = "")
            : this(paintNodeId, width, height, mimeType, encodedBytes, previewImageHash, false)
        {
        }

        /// <summary>Internal import path for an immutable source payload shared by several material nodes.</summary>
        internal GraphOriginalImage(string paintNodeId, int width, int height, string mimeType, byte[] encodedBytes, string previewImageHash, bool shareBytes)
        {
            ...
            PaintNodeId = paintNodeId; Width = width; Height = height; MimeType = mimeType; PreviewImageHash = previewImageHash ?? ""; this.encodedBytes = shareBytes ? encodedBytes : (byte[])encodedBytes.Clone(); ContentHash = Checks.Hash(this.encodedBytes);
        }
