...
MeshSource, Plane, EditMesh, Output, Scalar, `rig.skeleton`, `rig.skin-bind`, `rig.pose`, `rig.skin-deform` and metadata-only `object.attachment` v1 are implemented. `object.attachment` pins an accessory to an explicit target object, stable BoneId, skeleton hash and bone-local offset; runtime fitting and penetration correction remain outside this node. A scalar output may drive plane width/height in metres. Other types and future versions retain opaque payload in the in-memory graph and produce UNKNOWN_NODE; they are not executed. Wrong known port types, invalid edges and cycles are rejected when constructing a candidate graph, including disconnected components. Missing inputs are an incomplete evaluation, with independent node previews still available. This initial evaluator conservatively requires no diagnostics anywhere for IsComplete.
