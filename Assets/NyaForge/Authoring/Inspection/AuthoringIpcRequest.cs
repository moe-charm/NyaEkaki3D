...
namespace NyaForge.Authoring.Inspection
{
    public sealed class AuthoringIpcRequest
    {
        ...
        public static AuthoringIpcRequest Parse(byte[] bytes)
        {
            ...
            return new AuthoringIpcRequest { ImageImport=imageImport,RequestId=id,ExpectedInstanceId=instance,Method=method,Command=command,Save=save ? ProjectSaveWireReader.Read(r["save"] as JObject,instance) : null,Export=export ? ProjectExportRequest.Read(r["export"] as JObject) : null,Validation=validate ? AuthoringValidationRequest.Read(r["validation"] as JObject) : null,Vertices=vertices ? MeshPageRequest.Parse(r["vertices"] as JObject) : null,Faces=faces ? MeshPageRequest.Parse(r["faces"] as JObject,true) : null };
        }
    }
}
