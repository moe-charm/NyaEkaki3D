...
namespace NyaForge.Authoring.Import
{
    ...
    public sealed class VrmSpringSession
    {
        ...
        public bool HasCompleteDetails => SpringBones.All(group => group.Joints.All(joint => joint.GravityDirection.HasValue))
            && ColliderGroups.All(group => group.ColliderCount == 0 || (group.Shapes != null && group.Shapes.All(shape => shape.Offset.HasValue && (shape.Kind == "sphere" || shape.Tail.HasValue))));
    ...
    public static class VrmSpringSessionCodec
    {
        ...
        public static byte[] Write(VrmSpringSession session)
        {
            ...
            var groups = new JArray(session.SpringBones.Select(group => new JObject
            {
                ...
                ["joints"] = new JArray(group.Joints.Select(joint => new JObject { ["node"] = joint.NodeIndex, ["hitRadius"] = joint.HitRadius, ["stiffness"] = joint.Stiffness, ["gravityPower"] = joint.GravityPower, ["dragForce"] = joint.DragForce, ["gravityDirection"] = VrmSpringDetailJson.WriteVector(joint.GravityDirection) }))
            }));
            var colliderGroups = new JArray(session.ColliderGroups.Select(group => new JObject { ["node"] = group.NodeIndex, ["colliderCount"] = group.ColliderCount, ["nodes"] = new JArray(group.ColliderNodeIndices), ["shapes"] = VrmSpringDetailJson.WriteShapes(group.Shapes) }));
        ...
        public static VrmSpringSession Read(byte[] bytes)
        {
            ...
            try
            {
                ...
                foreach (var token in colliderArray) { var value = token as JObject; Checks.Require(value != null, "INVALID_VRM", "VRM SpringBone collider group is invalid."); RequireFields(value, version >= 3 ? new[] { "node", "colliderCount", "nodes", "shapes" } : new[] { "node", "colliderCount", "nodes" }); int node = Int(value, "node"); Checks.Require(node >= -1 && node < AuthoringLimits.MaxVertices, "INVALID_VRM", "VRM SpringBone collider node is out of range."); int count = Int(value, "colliderCount"); Checks.Require(count >= 0 && count <= VrmMetadata.MaxSpringColliders, "BUDGET_EXCEEDED", "VRM SpringBone collider count exceeds capacity."); var nodes = IntArray(value, "nodes", VrmMetadata.MaxSpringColliders, unique: false); Checks.Require(nodes.Count == count, "INVALID_VRM", "VRM SpringBone collider node count differs."); Checks.Require(nodes.All(item => item < AuthoringLimits.MaxVertices), "INVALID_VRM", "VRM SpringBone collider node is out of range."); colliders.Add(new VrmSpringColliderGroup(node, count, nodes, version >= 3 ? VrmSpringDetailJson.ReadShapes(value["shapes"], count) : null)); }
