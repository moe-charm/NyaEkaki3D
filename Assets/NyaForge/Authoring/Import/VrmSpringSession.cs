...
namespace NyaForge.Authoring.Import
{
    ...
    public sealed class VrmSpringSession
    {
        ...
        internal VrmSpringSession(string sourceHash, string format, string title, string author, IEnumerable<VrmSpringBoneGroup> springBones, IEnumerable<VrmSpringColliderGroup> colliderGroups)
        {
            ...
            SourceHash = sourceHash; Format = format; Title = title ?? ""; Author = author ?? ""; SpringBones = System.Array.AsReadOnly(bones); ColliderGroups = System.Array.AsReadOnly(colliders);
        }
        ...
        public static VrmSpringSession Create(VrmMetadata metadata)
        {
            ...
            return new VrmSpringSession(metadata.SourceHash, metadata.Format, metadata.Title, metadata.Author, metadata.SpringBones, metadata.SpringColliderGroups);
        }
    }
    ...
    public static class VrmSpringSessionCodec
    {
        ...
        public static byte[] Write(VrmSpringSession session)
        {
            ...
            var root = new JObject { ["version"] = Version, ["sourceHash"] = session.SourceHash, ["format"] = session.Format, ["title"] = session.Title, ["author"] = session.Author, ["springBones"] = groups, ["colliderGroups"] = colliderGroups };
        ...
        public static VrmSpringSession Read(byte[] bytes)
        {
            ...
            try
            {
                ...
                RequireFields(root, "version", "sourceHash", "format", "title", "author", "springBones", "colliderGroups"); Checks.Require(Int(root, "version") == Version, "UNSUPPORTED_FORMAT", "VRM SpringBone session version is unsupported.");
                string sourceHash = String(root, "sourceHash", 64); string format = String(root, "format", 32); string title = String(root, "title", 256, true); string author = String(root, "author", 256, true);
                ...
                return new VrmSpringSession(sourceHash, format, title, author, bones, colliders);
            }
        ...
        static IReadOnlyList<int> IntArray(JObject owner, string name, int maximumCount) { var token = owner[name] as JArray; Checks.Require(token != null && token.Count <= maximumCount, "INVALID_VRM", "VRM SpringBone session array is invalid: " + name); var result = new List<int>(); var seen = new HashSet<int>(); foreach (var item in token) { Checks.Require(item.Type == JTokenType.Integer, "INVALID_VRM", "VRM SpringBone session array item is invalid: " + name); int value = (int)item; Checks.Require(value >= 0 && seen.Add(value), "INVALID_VRM", "VRM SpringBone session array item is out of range: " + name); result.Add(value); } return result.AsReadOnly(); }
    }
}
