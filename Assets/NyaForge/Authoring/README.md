...
`Graph/` now owns typed node definitions, DAG validation, plane/source/edit/output evaluation, pinned edit contexts, and native graph persistence. `rig.skeleton` is a typed rest-skeleton node whose `NYRS` v1 blob survives save/reload, and `rig.skin-bind` validates mesh/skeleton identities while persisting an `NYRB` v1 binding blob; both are exposed by graph inspection. `rig.pose` and `rig.skin-deform` also persist and evaluate rest-relative deformation. `MorphTarget`/`MorphSet` provide a mesh-pinned sparse delta core and `NYRM` codec for future expression and shape-key adapters; they are not graph or GUI nodes yet. Existing static object evaluation passes through Source→EditMesh→Output; its historical property APIs are views over those nodes. Runtime Rig UI includes bone selection, weight mixing, brush weight paint, XYZ pose, rest movement and explicit rebind; real avatar import/export remain pending. See [Graph integration boundaries](Graph/README.md).
