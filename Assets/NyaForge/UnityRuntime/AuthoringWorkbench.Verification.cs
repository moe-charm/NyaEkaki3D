...
namespace NyaForge.UnityRuntime
{
    public sealed partial class AuthoringWorkbench
    {
        ...
        IEnumerator Verify(string output)
        {
            ...
            yield return VerifyMaterialUi(output,error=> { if(error!=null) failure=(failure ?? "")+"\n"+error; else checks.Add("Material GUI: create preserving Paint, semantic normal/MR file import with UV/scale, native Save/Open and scalar retention, atomic migration Undo/Redo, validated values, unchanged precision, 3D paint through material, and adding Paint to an untextured material"); });
