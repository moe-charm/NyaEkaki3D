    ...
    public sealed partial class AuthoringWorkbench
    {
        ...
        IEnumerator Verify(string output)
        {
            ...
            try
            {
                ...
                VerifyMorphTargetIdentity(checks);
                ...
                checks.Add("status footer retains complete text in its tooltip and may wrap on a narrow window");
            }
            catch (Exception e) { failure = e.ToString(); Debug.LogException(e); }
            ...
            yield return VerifyMaterialUi(output,error=> { if(error!=null) failure=(failure ?? "")+"\n"+error; else checks.Add("Material GUI: create preserving Paint, reject unsupported UV1 semantic maps, apply UV0 normal/MR files with scale, native Save/Open and scalar retention, atomic migration Undo/Redo, validated values, unchanged precision, 3D paint through material, and adding Paint to an untextured material"); });
