...
実SDKを入れた受け取り側を確認する前に、Unity projectを変更せず依存とcomponent候補を調べるPowerShell preflightを実行できます。

```powershell
.\Tools\Test-NyaForgePhysBonesSdk.ps1 -UnityProjectPath 'C:\path\to\receiver' `
  -TargetManifestPath 'C:\path\to\physbones.nyaforge-target.json' `
